...
covers as (
  ...
  select
    ...
    case
      ...
      when cover_owner = 0x8b86cf2684a3af9dd34defc62a18a96deadc40ff and product_type = 'The Retail Mutual' then 'TRM'
      ...
      when cover_owner = 0x2557fe0959934f3814c6ee72ab46e6687b81b8ca and product_type = 'Follow On Cover' then 'Ensuro'
      --when ... then 'Native'
      else 'Nexus Mutual'
    end as seller
...
daily_cover_sales as (
  select
    ...
    --cbBTC
    case
      when c_start.premium_asset = 'cbBTC' then coalesce(c_start.nxm_premium_amount * p.avg_nxm_usd_price / p.avg_eth_usd_price, 0)
      else 0
    end as cbbtc_eth_premium,
    case
      when c_start.premium_asset = 'cbBTC' then coalesce(c_start.nxm_premium_amount * p.avg_nxm_usd_price, 0)
      else 0
