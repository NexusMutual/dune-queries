WITH
 ...
 erc_transactions AS (
  ...
  select
    ...
    running_eth_egress,
    running_eth_ingress,
    COALESCE(running_net_maple, 0) as running_maple,
    SUM(COALESCE(dai_net_total, 0)) OVER (ORDER BY price_ma.day) as running_dai,
    COALESCE(lido_ingress, 0) as running_lido
  from
    price_ma
...
SELECT
  ...
  case
    ...
    when '{{display_currency}}' = 'ETH' then moving_average_dai * running_net_lido / moving_average_eth
    ELSE -1
  END as running_net_lido_display_curr,
  case
    when '{{display_currency}}' = 'USD' then moving_average_eth * running_net_maple
    when '{{display_currency}}' = 'ETH' then moving_average_dai * running_net_maple / moving_average_eth
    ELSE -1
  END as running_net_maple_display_curr,
  case
    when '{{display_currency}}' = 'USD' then (moving_average_dai * running_net_dai) + (
