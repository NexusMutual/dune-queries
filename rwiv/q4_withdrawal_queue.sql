-- RWIV Q4 - Withdrawal Queue
-- Dune query ID: 6973373
-- Tracks all withdrawal requests and their status in the RWIV vault.
-- Request size in USDC is calculated from the rate model (RedeemRequested only emits shares).
-- Fulfilment values read directly from RedeemFulfilled event emissions.
...
SELECT
  ...
  address,
  CASE
    WHEN fulfilled_shares >= requested_shares THEN 'Filled'
  ...
  ROUND(
    CASE
      WHEN fulfilled_shares >= requested_shares THEN 0e0
      WHEN cancel_time IS NOT NULL THEN 0e0
      ELSE (requested_shares - fulfilled_shares)
        * start_rate
