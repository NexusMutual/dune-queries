  ...
  select
    map(
      ...
      array[
        ...
        format('%.2f%%', cast(renewal_cover_buyers * 100.0 / total_cover_buyers as double)),

        cast(null as varchar),
        format('%.2f', cast(avg_coverage_lifetime_months as double)),
        format('%.2f', cast(avg_covers_per_buyer as double)),
        format('%.2f', cast(median_covers_per_buyer as double)),
        cast(max_covers_by_single_buyer as varchar),
        format('%.2f', cast(longest_continuous_coverage_months as double)),
        format('%.2f', cast(avg_streaks_per_user as double)),

        cast(null as varchar),
        format('%.2f%%', cast(pct_buyers_with_3plus_months * 100.0 as double)),
        format('%.2f%%', cast(pct_buyers_with_12plus_months * 100.0 as double)),

        cast(null as varchar),
        format('%.2f', cast(avg_days_between_covers as double)),
        format('%.2f', cast(median_days_between_covers as double)),
        format('%.2f', cast(gs.global_median_renewal_gap as double)),
        format('%.2f', cast(gd.avg_gap_stddev as double)),
        format('%.2f', cast(avg_final_gap_days as double)),
        format('%.2f', cast(avg_gap_count_per_buyer as double)),

        cast(null as varchar),
        format('%.2f%%', cast(gd.pct_renewals_with_gap_1_7d * 100.0 as double)),
        format('%.2f%%', cast(gd.pct_renewals_with_gap_8_30d * 100.0 as double)),
        format('%.2f%%', cast(gd.pct_renewals_with_gap_31_90d * 100.0 as double)),
        format('%.2f%%', cast(gd.pct_renewals_with_gap_gt_90d * 100.0 as double)),

        cast(null as varchar),
        format('%.2f%%', cast(pct_buyers_with_max_gap_30d * 100.0 as double)),
        format('%.2f%%', cast(pct_buyers_with_max_gap_90d * 100.0 as double)),
        format('%.2f%%', cast(pct_buyers_with_all_zero_gaps * 100.0 as double)),
        format('%.2f%%', cast(pct_buyers_with_all_nonzero_gaps * 100.0 as double)),
        format('%.2f%%', cast(pct_buyers_with_mixed_gaps * 100.0 as double)),
        format('%.2f%%', cast(pct_buyers_with_final_gap_gt_30d * 100.0 as double)),
        format('%.2f%%', cast(pct_buyers_with_gap_but_no_overlap * 100.0 as double))
      ]
    ) as metrics
