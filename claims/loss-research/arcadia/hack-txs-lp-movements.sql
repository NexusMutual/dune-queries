...
asset_movements as (
  select
    ...
    e.evt_tx_from as account,
    e.owner as sub_account,
    e.assets / 1e6 as usdc_amount,
    e.evt_index,
    e.evt_tx_hash as tx_hash
  from arcadia_v2_base.usdc_lp_evt_deposit e
    inner join address_to_check a on e.owner = a.sub_account and e.evt_tx_hash = a.hack_tx_hash
  union all
  select
    a.cover_id,
    e.evt_block_time as block_time,
    ...
    e.evt_tx_from as account,
    e.account as sub_account,
    e.amount / 1e6 as usdc_amount,
    e.evt_index,
    e.evt_tx_hash as tx_hash
  from arcadia_v2_base.usdc_lp_evt_borrow e
    inner join address_to_check a on e.account = a.sub_account and e.evt_tx_hash = a.hack_tx_hash
  union all
  select
    a.cover_id,
    e.evt_block_time as block_time,
    ...
    e.evt_tx_from as account,
    e.account as sub_account,
    -1 * (e.amount / 1e6) as usdc_amount,
    e.evt_index,
    e.evt_tx_hash as tx_hash
  from arcadia_v2_base.usdc_lp_evt_repay e
    inner join address_to_check a on e.account = a.sub_account and e.evt_tx_hash = a.hack_tx_hash
  union all
  select
    a.cover_id,
    ...
    e.evt_tx_from as account,
    e.owner as sub_account,
    -1 * (e.assets / 1e6) as usdc_amount,
    e.evt_index,
    e.evt_tx_hash as tx_hash
  from arcadia_v2_base.usdc_lp_evt_withdraw e
    inner join address_to_check a on e.owner = a.sub_account and e.evt_tx_hash = a.hack_tx_hash
)
