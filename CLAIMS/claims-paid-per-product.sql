...
reimbursements (block_number, product_name, amount, symbol, tx_hash) as (
  ...
  (19009119, 'FTX', 25218, 'USDC', 0x2b908780406a06ef8f936c3b801ff722a0f0ba36d00699f53ad64f24260d6b45),
  ...
  (18643871, 'FTX', 4800, 'USDC', 0xb699014515bfce28843dd997f48d015f7373bf8a6887d3fcd411ce90005795bf),
...
reimbursement_ext as (
  select
    block_date,
    product_name,
    -1 * if(symbol in ('ETH', 'WETH'), amount_eth, 0) as eth_eth_reimbursement_amount,
    -1 * if(symbol in ('ETH', 'WETH'), amount_usd, 0) as eth_usd_reimbursement_amount,
    -1 * if(symbol = 'DAI', amount_eth, 0) as dai_eth_reimbursement_amount,
    -1 * if(symbol = 'DAI', amount_usd, 0) as dai_usd_reimbursement_amount,
    -1 * if(symbol = 'USDC', amount_eth, 0) as usdc_eth_reimbursement_amount,
    -1 * if(symbol = 'USDC', amount_usd, 0) as usdc_usd_reimbursement_amount
...
select
  ...
  sum(if('{{display_currency}}' = 'USD', eth_usd_reimbursement_amount, eth_eth_reimbursement_amount)) as eth_claim_total,
  sum(if('{{display_currency}}' = 'USD', dai_usd_reimbursement_amount, dai_eth_reimbursement_amount)) as dai_claim_total,
  sum(if('{{display_currency}}' = 'USD', usdc_usd_reimbursement_amount, usdc_eth_reimbursement_amount)) as usdc_claim_total,
  sum(if(
    '{{display_currency}}' = 'USD',
