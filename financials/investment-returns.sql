...
capital_pool as (
  select
    ...
    lead(avg_usdc_eth_total, 1) over (order by block_date desc) as usdc_prev,
    ...
    lead(avg_cover_re_usdc_eth_total, 1) over (order by block_date desc) as cover_re_prev,
...
investment_returns as (
  select
    ...
    coalesce((dai - dai_prev) / nullif(dai_prev, 0), 0) as dai_pct,
    coalesce(power(1 + ((dai - dai_prev) / nullif(dai_prev, 0)), 12) - 1, 0) as dai_apy,
    usdc,
    usdc_prev,
    usdc - usdc_prev as usdc_return,
    coalesce((usdc - usdc_prev) / nullif(usdc_prev, 0), 0) as usdc_pct,
    coalesce(power(1 + ((usdc - usdc_prev) / nullif(usdc_prev, 0)), 12) - 1, 0) as usdc_apy,
    cbbtc,
    cbbtc_prev,
    ...
    coalesce((cover_re - cover_re_prev) / nullif(cover_re_prev, 0), 0) as cover_re_pct,
    coalesce(power(1 + ((cover_re - cover_re_prev) / nullif(cover_re_prev, 0)), 12) - 1, 0) as cover_re_apy,
    aweth,
    aweth_prev,
...
select
  ...
  coalesce(power(1 + ((aweth_return + debt_usdc_return) / nullif(aweth_prev, 0)), 12) - 1, 0) as aweth_apy,
