...
select
  ...
  case i.label
    -- revenue
    when 'Revenue Statement' then null
    when 'Cash Surplus' then usd_premium + usd_member_fee + usd_claim_paid + usd_reimbursement
    ...
    when 'Total Cash Movement' then usd_premium + usd_member_fee + usd_claim_paid + usd_reimbursement + usd_inv_returns + usd_fx_change + usd_eth_in + usd_eth_out
  end as usd_val
