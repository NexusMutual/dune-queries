...
stakers as (
  select
    ...
    listagg(distinct cast(pool_id as varchar), ',') within group (order by cast(pool_id as varchar)) as pools,
    listagg(distinct cast(token_id as varchar), ',') within group (order by cast(token_id as varchar)) as tokens
