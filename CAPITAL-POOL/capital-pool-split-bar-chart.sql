...
capital_pool_assets (asset_type, eth_amount, usd_amount, rn) as (
  ...
  select 'cbBTC', avg_cbbtc_eth_total, avg_cbbtc_usd_total, 7 from capital_pool_timed union all
  select 'Cover Re USDC', avg_cover_re_usdc_eth_total, avg_cover_re_usdc_usd_total, 8 from capital_pool_timed union all
  select 'Aave collateral WETH', aave_collateral_weth_total, avg_aave_collateral_weth_usd_total, 9 from capital_pool_timed union all
  select 'Aave debt USDC', avg_aave_debt_usdc_eth_total, avg_aave_debt_usdc_usd_total, 10 from capital_pool_timed
)
